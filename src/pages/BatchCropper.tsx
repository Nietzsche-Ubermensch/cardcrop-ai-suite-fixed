...
const BatchCropper: React.FC<BatchCropperProps> = ({ initialFiles, folderName, onClearInitialFiles }) => {
   ...
   * firing every item through Promise.all() simultaneously. Each card render here
   * creates its own WebGL context (see webglRenderer.ts / exportCroppedHighRes);
   * browsers cap simultaneous WebGL contexts well under MAX_BATCH_SIZE (100), so an
   * unbounded Promise.all was silently losing contexts partway through large
   * batches — visible as the batch stalling / "not moving" with no error surfaced.
   */
  ...
  // Stay comfortably under the browser's simultaneous-WebGL-context ceiling
  // (commonly ~8-16 depending on browser/GPU) while still processing in parallel.
  const WEBGL_BATCH_CONCURRENCY = 4;
  ...
  const handleApplyEnhancementsToAll = async () => {
    ...
    const results = await runWithLimit(cards, WEBGL_BATCH_CONCURRENCY, async (card) => {
      try {
        const img = new Image();
  ...
  const handleDownloadBatchZip = async () => {
    ...
    try {
      ...
      await runWithLimit(cards, WEBGL_BATCH_CONCURRENCY, async (card, idx) => {
        let exportBlob: Blob;
